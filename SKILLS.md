...
10. **Selling estimation deducts fee conservatively.** The `FEE()` call is wrapped in try-catch: if the terminal does not implement `IJBFeeTerminal`, the fee defaults to 0 (no fee deducted). For standard terminals, the protocol fee is always deducted. If the hook is feeless, the estimate is slightly low, routing to V4 instead.
