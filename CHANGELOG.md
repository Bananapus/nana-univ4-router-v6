...
- `calculateExpectedOutputFromSelling` no longer deducts the terminal fee a second time on metadata-only sell previews. When `previewCashOutFrom` returns `reclaimAmount == 0` and the buyback hook spec carries an executable `minimumSwapAmountOut` inside metadata, that amount is the AMM sell-side proceeds, which already bypass `_processFee` (the sell-side hook spec is built with `amount = 0`). Subtracting `terminal.FEE()` again unconditionally double-discounted the metadata route and silently pushed it below the V4 quote even when execution would have paid more. The fix skips the fee deduction when `grossReclaim == 0`; the standard non-zero reclaim path is unchanged. (Released in `0.0.29`.)
