...
contract TestStructuralArbitrage is Test {
    ...
    // ============================================
    // Test 9: Zero cashOutTaxRate skips JB routing
    // ============================================

    /// @notice When a project has cashOutTaxRate == 0, the bonding curve imposes no penalty on
    /// cashouts: every token redeems for its exact proportional share of surplus. This creates
    /// a structural arbitrage where repeated cashouts extract surplus without convergence.
    /// The fix: the hook detects zero-tax projects and skips JB routing entirely, letting
    /// the V4 AMM handle swaps. This test proves the fix works.
    function test_structuralArbitrage_zeroTaxRate_skipsJBRouting() public {
        // Configure a ZERO cashOutTaxRate for the project.
        // The mock controller already defaults to 0, but we set it explicitly for clarity.
        mockJbController.setCashOutTaxRate(PROJECT_ID, 0);

        // Configure the bonding curve store with zero tax and surplus that makes JB output > V4.
        // With 0% tax, reclaim = surplus * count / supply = 500 * 5 / 100 = 25 ETH per 5 tokens.
        // V4 at 1:1 would give ~5 ETH. So JB would be "better" without the zero-tax guard.
        terminalStore.configure(INITIAL_SURPLUS, INITIAL_SUPPLY, 0);

        // Add deep V4 liquidity so swaps routed to V4 don't exhaust the pool.
        token0.mint(address(this), 10_000 ether);
        token1.mint(address(this), 10_000 ether);
        token0.approve(address(modifyLiquidityRouter), 10_000 ether);
        token1.approve(address(modifyLiquidityRouter), 10_000 ether);
        modifyLiquidityRouter.modifyLiquidity(
            key,
        ...
        // Reset call count to isolate our measurement.
        // (setUp may have triggered calls during pool init.)
        uint256 callCountBefore = terminal.callCount();

        // Execute multiple sell-side swaps.
        uint256 numSwaps = 5;
        for (uint256 i = 0; i < numSwaps; i++) {
            _sellJbTokens(SWAP_SIZE);
        }

        uint256 cashoutsDuringTest = terminal.callCount() - callCountBefore;

        console.log("--- Zero Tax Rate: Skip JB Routing ---");
        console.log("  cashOutTaxRate:   0");
        console.log("  Swaps executed:   %d", numSwaps);
        console.log("  JB cashouts:      %d", cashoutsDuringTest);

        // With the fix, zero-tax projects should NOT be routed through JB.
        assertEq(cashoutsDuringTest, 0, "Zero-tax projects must not be routed through JB cashout");
    }
}
